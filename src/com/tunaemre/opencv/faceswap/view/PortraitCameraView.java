...
public class PortraitCameraView extends PortraitCameraBridgeViewBase implements PreviewCallback
{
		...
		synchronized (this)
		{
			...
			try
			{
				...
				if (sizes != null)
				{
					...
					if ((getLayoutParams().width == LayoutParams.MATCH_PARENT) && (getLayoutParams().height == LayoutParams.MATCH_PARENT)) mScale = Math.min(((float) height) / mFrameWidth, ((float) width) / mFrameHeight);
	...
	public int switchCamera()
	{
		...
		if (this.mCameraIndex == CAMERA_ID_BACK || this.mCameraIndex == CAMERA_ID_ANY) mCameraIndex = CAMERA_ID_FRONT;
		else
